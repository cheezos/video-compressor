...
class Main(QMainWindow):
    def __init__(self, parent=None) -> None:
        ...
        self.button_select_videos.move(10, 60)
        ...
        self.ledit_video_list.setPlaceholderText(
            "Browse and select videos to compress."
        )
        self.ledit_video_list.move(round(self.width * 0.33 + 10), 60)
        ...
        self.label_file_size.move(10, 95)
        ...
        self.ledit_file_size.move(round(self.width * 0.33 + 10), 95)
        ...
        self.label_mb.move(round(self.width * 0.33 + 70), 95)
        ...
        self.label_remove_audio.move(10, 130)
    ...
    def init_config(self) -> None:
        ...
        if os.path.exists(os.getcwd() + "/config.ini"):
            ...
            self.ffmpeg_path = self.config.get("Options", "ffmpeg_path")
            self.ffmpeg_download_link = self.config.get(
                "Options", "ffmpeg_download_link"
            )
            self.ffmpeg_download_path = self.config.get(
                "Options", "ffmpeg_download_path"
            )
    ...
    def set_config(self) -> None:
        self.config.set("Options", "ffmpeg_path", self.ffmpeg_path)
        self.config.set("Options", "ffmpeg_download_link", self.ffmpeg_download_link)
        self.config.set("Options", "ffmpeg_download_path", self.ffmpeg_download_path)
        self.config.set(
    ...
    def init_ffmpeg(self) -> None:
        if os.path.exists(os.getcwd() + "/ffmpeg/ffmpeg.exe") and os.path.exists(
            ...
            self.worker = Worker(
                self.ffmpeg_path, self.ffmpeg_download_link, self.ffmpeg_download_path
            )
        ...
        if not self.compressing:
            if len(self.selected_videos) > 0:
                if not self.ffmpeg_installed:
                    ...
                    self.worker = Worker(
                        self.ffmpeg_path,
                        self.ffmpeg_download_link,
                        self.ffmpeg_download_path,
